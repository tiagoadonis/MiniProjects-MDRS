function [PLdata , APDdata, MPDdata , TT , PLvoip, APDvoip, MPDvoip ] = Simulator4New(lambda,C,f,P,n)
% INPUT PARAMETERS:
...
QUEUEVOIPOCCUPATION = 0;    % Occupation of the voip queue (in Bytes)
        ...
        case ARRIVAL            % If first event is an ARRIVAL
            if PacketType == DATA
                ...
                if STATE == 0
                    ...
                    if QUEUEOCCUPATION + QUEUEVOIPOCCUPATION <= f*0.9 && QUEUEOCCUPATION + QUEUEVOIPOCCUPATION + PacketSize
                ...
                if STATE == 0
                    ...
                    if QUEUEOCCUPATION + PacketSize <= f
                        QUEUEVOIP = [QUEUEVOIP; PacketSize , Clock, PacketType];
                        QUEUEVOIPOCCUPATION = QUEUEVOIPOCCUPATION + PacketSize;
                    else
...
APDvoip = 1000*VOIPDELAYS/TRANSMITTEDVOIPPACKETS;   % in milliseconds
MPDdata = 1000*MAXDELAY;                            % in milliseconds
MPDvoip = 1000*MAXVOIPDELAY;                        % in milliseconds
